...
# model.addConstr(u[(8,12)] == 1)  
# model.addConstr(u[(12,1)] == 1)
# model.addConstr(u[(1,0)] == 1)
# model.addConstr(u[(0,15)] == 1)
# model.addConstr(u[(15,3)] == 1)
########################################


model.addConstr(quicksum(u[(ds,j)] for j in Lt[ds]|{de}) == 1)
...
model.addConstr(quicksum(u[(j,de)] for j in Lse-{de}) == 1)
...
if len(Rd) != 0:
    ...
    for i in Lse:
        Feasibleset = Lt[i]|set(Rmap[i])|{de}
...
for i in R:
    ...
    for j in Su:
        model.addConstr(T[j] + B*(u[(i,j)] - 1) <= T[fd[i]])
...
## 在必须备餐完毕才可取餐
for i in R:
    model.addConstr(quicksum(x[(j,i)]*T[i] for j in (Nse|Su))+quicksum(u[(j,i)]*TD[i] for j in L) >= aa[i])
# for i in R:
#     model.addConstr(B*(1-quicksum(x[(j,i)] for j in (Nse|Su)) +T[i] ) >= aa[i])
#     model.addConstr(B*(1-quicksum(u[(j,i)] for j in L) + TD[i] ) >= aa[i])
...
### 构造目标函数并求解
for i in range(n):
    model.addConstr(timeDifference[i] == (T[i]-bb[i]))
    model.addGenConstrMax(lateTime[i],[timeDifference[i],0])
